...
class LaravelEditorJs
{
    ...
    public function render_amp(string $data) : string
    {
        try {
            $configJson = json_encode(config('laravel_editorjs.config') ?: []);
            ...
            foreach ($editor->getBlocks() as $block) {
                ...
                $viewName = "laravel_editorjs::ampblocks." . Str::snake($block['type'], '-');
    ...
    public function render_turbo(string $data) : string
    {
        try {
            $configJson = json_encode(config('laravel_editorjs.config') ?: []);
            ...
            foreach ($editor->getBlocks() as $block) {
                ...
                $viewName = "laravel_editorjs::turboblocks." . Str::snake($block['type'], '-');
