<figure class="editor-js-image{{ $data['classes'] ?? ' '}}">
    ...
    <a href="{{str_replace(['s3.cs.ixbt.com','storage.ixbt.site','s3f.ixbt.site','storage.ixbt.com','s3.ixbt.site'],'cdn.ixbt.site',$data['file']['url'])}}" class="glightbox"
       @if($data['caption'] ?? null) data-title="{{$data['caption']}}" @endif>
        <img @if(!empty($data['file']['width']) && $data['file']['width'] != 0)width="{{$data['file']['width']}}"
             ...
             decoding="async" src="{{ thumbor(str_replace(['s3.cs.ixbt.com','storage.ixbt.site','s3f.ixbt.site','storage.ixbt.com','s3.ixbt.site'],'cdn.ixbt.site',$data['file']['url']),736) }}"
