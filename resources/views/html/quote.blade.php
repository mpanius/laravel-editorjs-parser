<div class="editor-js-block border-amber-50 border rounded-xl bg-amber-50">
    <blockquote class="relative border-0 not-prose" style="margin-top: 0.5rem; margin-bottom: 0.5rem;">
        ...
        <div class="relative">
            <div class="text-gray-600 dark:text-white font-light">{{$data['text'] ?? ''}}</div>
        </div>
        @if(!empty($data['caption']))
            <footer class="mt-2 flex">
                <div class="flex text-gray-600 dark:text-gray-400 ml-auto mr-12 not-italic"> {{$data['caption']}}</div>
            </footer>
        @endif
